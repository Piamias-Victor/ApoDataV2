...
export const Drawer: React.FC<DrawerProps> = ({
    ...
    // Use portal to break out of parent stacking context
    const content = (
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    {/* PanelContainer to ensure z-index applies */}
                    <div className="fixed inset-y-0 right-0 z-[10000] flex">
                        <motion.div
                            className={`relative h-full ${width} bg-white/90 backdrop-blur-2xl shadow-2xl flex flex-col border-l border-white/50`}
                            initial={{ x: '100%' }}
                            animate={{ x: 0 }}
                            exit={{ x: '100%' }}
