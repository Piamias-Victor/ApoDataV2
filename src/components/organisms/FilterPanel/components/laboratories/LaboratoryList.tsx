...
export const LaboratoryList: React.FC<LaboratoryListProps> = ({
    items, isLoading, hasSelection, searchQuery, mode, selectedMap, onToggle
}) => {
    const hasResults = items.length > 0;

    if (isLoading) {
        return <FilterLoadingState message="Chargement des laboratoires/marques..." color="purple" />;
    }
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3 overflow-hidden">
                                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-400 group-hover:bg-white group-hover:text-purple-500'
                                        }`}>
                                        {mode === 'laboratory' ? <TestTube className="w-5 h-5" /> : <Tag className="w-5 h-5" />}
                                    </div>
                                    <div className="min-w-0">
                                        <h4 className={`text-sm font-bold truncate ${isSelected ? 'text-purple-900' : 'text-gray-700'}`}>
                                            {lab.laboratory_name}
                                        </h4>
