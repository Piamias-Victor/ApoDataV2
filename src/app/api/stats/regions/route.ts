...
export async function POST(req: NextRequest) {
    try {
        ...
        qb.addCategories(categories.map(c => ({ code: c.id, type: c.type })));
        ...
        if (excludedCategories.length) qb.addExcludedCategories(excludedCategories.map(c => ({ code: c.id, type: c.type })));
        ...
        return NextResponse.json(result.rows.map((row: any) => ({
            ...
            averageSales: Number(row.pharmacy_count) > 0 ? Number(row.total_sales) / Number(row.pharmacy_count) : 0
        })));
